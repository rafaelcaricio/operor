...
import plp.operor.expression.Aplicacao;
...
/**
 * @author S�rgio
 * @author Rafael Fons�ca (_visitOperacao)
 * 
...
public class PartialInstantiatorVisitor {
	...
	public Expressao _visitAplicacao(Aplicacao expressao,
		...
		Aplicacao resultado = new Aplicacao(func.avaliar(ambiente),
				novosValoresReais);

		return resultado;
	}

	public Expressao _visitOperacao(Operacao expressao, AmbienteExecucao ambiente,
			Set<Id> localVariables) {
		ListIdOperador<Expressao> listaParam = new ListIdOperador<Expressao>();
		for (Object o : expressao.getListaOperadores().getListaOrdenacao()) {
			if (o instanceof IdOperador) {
				listaParam.addIdOperador((IdOperador)o);
			} else {
				listaParam.addElemento(visit((Expressao)o, ambiente, localVariables));
			}
		}
		Operacao resultado = new Operacao(listaParam);
		return resultado;
	}
	...
	public Expressao _visitIdFuncaoVariavel(Id thisId, AmbienteExecucao ambiente,
			Set<Id> localVariables) {
		Expressao resultado;
		if (localVariables.contains(thisId)) {
			resultado = thisId;
	...
	public Expressao _visitValorFuncao(ValorFuncao valor,
		...
		Expressao novaExpressao = visit(valor.getExp(), ambiente,
				novasVariaveisLocais);
		ValorFuncao resultado = new ValorFuncao(valor.getListaId(),
				novaExpressao);
		resultado.setOpCriadores(valor.getOpCriadores());
		return resultado;
	}
	...
	public Expressao _visitExpSoma(ExpSoma expressao,
			AmbienteExecucao ambiente, Set<Id> localVariables) {
		Expressao esquerda = visit(expressao.getEsq(), ambiente, localVariables).avaliar(ambiente);
		Expressao direita = visit(expressao.getDir(), ambiente, localVariables).avaliar(ambiente);
		Expressao resultado = new ExpSoma(esquerda, direita);
		return resultado;
	}

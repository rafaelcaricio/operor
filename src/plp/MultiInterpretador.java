...
import plp.operor.parser.OperORParser;
...
public class MultiInterpretador {
	...
	private static final int OPEROR = 8;
	...
	private OperORParser operorParser = null;
	...
	private void interpretarOperOR(ByteArrayInputStream fis) throws Exception {
		plp.operor.Programa prog;
		if (operorParser == null) {
			operorParser = new OperORParser(fis);
		} else {
			OperORParser.ReInit(fis);
		}
		prog = OperORParser.Input();
